...
public class FlyBehaviour : GenericBehaviour
{
	...
	public float flySpeed = (4.0f / (float)ReceivePosition.gammaScore);                 // Default flying speed.
	...
	void Update()
	{
        ...
        if (ReceivePosition.gammaScore <= 3 && (Time.frameCount % 2) == 0)
		{
			fly = true;
        ...
        if (ReceivePosition.gammaScore > 3)
        {
            fly = false;
